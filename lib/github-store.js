...
/**
 * The identity every server-made commit is authored by. CI matches on this to
 * enforce, from inside the workflow, what the market credential may write.
 */
export const MARKET_AUTHOR = { name: 'wrong.aecs.io market server', email: 'market@wrong.aecs.io' };
...
export class GitHubStore {
  constructor({ repo, branch, token, fetchFn = globalThis.fetch, cacheMs = 20000 } = {}) {
    ...
    // response. Capturing it turns a credential that dies quietly in a month
    // into one that announces itself beforehand.
  ...
  /**
   * Prove the credential actually works, rather than merely existing.
   * "A token string is set" and "writes will succeed" are different claims, and
   * only the second one matters when the thing is unattended.
   */
  async checkCredential() {
    if (!this.token) return { ok: false, reason: 'no token configured' };
    try {
      const meta = await this.#request(`/repos/${this.repo}`);
      if (!meta) return { ok: false, reason: `repository ${this.repo} not visible to this token` };
      const out = { ok: true, repo: this.repo, branch: this.branch };
      if (this.tokenExpiry) {
        const days = Math.floor((Date.parse(this.tokenExpiry) - Date.now()) / 86400000);
        out.token_expires = this.tokenExpiry;
        out.token_expires_in_days = days;
        if (days <= 14) out.warning = `The market credential expires in ${days} day(s). Orders and registrations will start failing when it does.`;
      }
      return out;
    } catch (err) {
  ...
  async #put(path, content, { sha = null, message }) {
    ...
    // Every write is attributed to a distinct identity, so CI can tell a commit
    // made by the market server apart from one made by a human or a routine.
    // That is what lets the workflow police what this credential is allowed to
    // touch - see the "market server writes" step in ci.yml.
    const body = {
      message,
      content: Buffer.from(content, 'utf8').toString('base64'),
      branch: this.branch,
      author: { name: MARKET_AUTHOR.name, email: MARKET_AUTHOR.email },
    };
    if (sha) body.sha = sha;
