...
export function liveness(market, { now = new Date() } = {}) {
  ...
  // A canary round that has closed with orders in it and has no clearing record
  // is invisible above, because that loop reads clearing records and this round
  // has none. So on the first day this market ever had orders, the check went on
  // quoting the previous three canaries - "0 distinct seat(s) submitted; 2
  // required to clear" - while the actual state was two seats submitted and a
  // runner that could not open them.
  //
  // Those are opposite problems. One says nobody is turning up; the other says
  // they turned up and the clearing engine could not read their orders. Pointing
  // at the wrong one sends whoever reads this to fix a thing that is not broken,
  // which is worse than saying nothing.
  //
  // This changes only what the check SAYS, never whether it fails. A round that
  // shut twenty minutes ago and has not cleared yet is perfectly normal - the
  // clearing job runs on its own cadence - so treating that as a fault would cry
  // wolf daily. The `clearing` check above already covers a round left unclear
  // too long, on a threshold chosen for that job's schedule.
  const blockedCanaries = (market.roundsAwaitingClear ?? []).filter(
    ({ question, round }) => (question.lane ?? 'standard') === 'canary' && (round.commitment_count ?? 0) > 0,
  );
  ...
  if (recent.length) {
    ...
    const liveReason =
      `${blockedCanaries.length} canary round(s) closed with orders in them and have not been cleared ` +
      `(${blockedCanaries.map(({ question, round }) => `${question.id}/${round.id}: ${round.commitment_count} order(s)`).join(', ')}). ` +
      'Seats are turning up; the orders are not being turned into a price.';

    add('canary_clears', 'Canary rounds clear', clearedCount > 0,
      clearedCount > 0
        ? `${clearedCount} of the last ${recent.length} canary round(s) cleared.`
        : blockedCanaries.length
          ? liveReason
          : staleReason);
  }
