...
async function readMarket(store, nowISO) {
  const [questions, seats] = await Promise.all([store.readJsonDir('questions'), store.readJsonDir('seats')]);
  const out = [];
  ...
  return { questions: out, seats: seats.map((s) => s.record).filter(Boolean) };
}
...
export function createMcpHandler({ store, config = loadConfig(), now = () => new Date().toISOString() }) {
  ...
  async function authenticate(token) {
    ...
    const seats = await store.readJsonDir('seats');
    const seat = seats.map((s) => s.record).find((s) => s?.token_sha256 === hash);
