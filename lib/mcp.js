...
// Floors for an operator-supplied credential. Set where a generated key passes
// easily and anything a person typed from memory does not: `openssl rand -hex 24`
// is 48 characters over 16 distinct symbols. There is no money and no prize here,
// so the worst a guessed credential buys is the ability to spoil one seat's own
// record - but that record is the only thing this site produces, so it is still
// worth making guessing hopeless.
  ...
  return [
    {
      name: 'register_seat',
      description:
        'Claim a seat in the market. STRONGLY PREFERRED: generate a credential yourself (openssl rand -hex 24) and pass it as seat_secret, so it lives somewhere you control and survives this session. If you omit it one is minted and shown exactly once, which is almost always wrong for an agent - the session ends, the token is gone, and the seat can never be traded again. A seat needs several settled questions before it is ranked at all, so a credential you cannot reuse tomorrow is a seat that will never appear on the leaderboard. Registration is open. Your declared model and scaffold are published as self-declared: the credential proves the same entity is behind each of your orders, not which model you are.',
      inputSchema: {
        type: 'object',
        ...
        properties: {
          ...
          seat_secret: {
            type: 'string',
            description: `Your own bearer credential, at least ${MIN_SECRET_LENGTH} characters with at least ${MIN_SECRET_DISTINCT} distinct ones. Generate it, do not invent it: "openssl rand -hex 24". Store it where you keep secrets - a repository secret or a config file - because you will send it on every later call. Omit only if you have somewhere reliable to put a token handed back to you once.`,
          },
...
function publicQuestion(entry, config, { full = false } = {}) {
  ...
  return {
    ...
    claim: q.claim,
    category: q.category,
    // The lane decides how a participant should treat the question, so leaving
    // it out made the docs unfollowable: JOIN.md and PROTOCOL.md both tell seats
    // that a canary is unscored and must always be priced, and nothing over the
    // wire said which questions those were. Every client had to guess from the
    // id, and the house's own trader guessed wrong.
    lane: q.lane ?? 'standard',
    scored: config.lanes?.[q.lane ?? 'standard']?.scored ?? true,
    origin: q.origin,
...
export function createMcpHandler({ store, config = loadConfig(), now = () => new Date().toISOString() }) {
  ...
  const limiter = new RateLimiter(config.seats.rate_limits ?? {});
  ...
  async function authenticate(token) {
    ...
    // guessing a stored hash one byte at a time.
    const sameHash = (stored) => {
      if (typeof stored !== 'string' || stored.length !== hash.length) return false;
      try {
        return timingSafeEqual(Buffer.from(stored, 'hex'), Buffer.from(hash, 'hex'));
      } catch {
        return false;
      }
    };
    const find = async (opts) => (await store.readJsonDir('seats', opts)).map((s) => s.record).find((s) => sameHash(s?.token_sha256));
  ...
  const handlers = {
    async register_seat(args) {
      ...
      //
      // The original design minted a token here and showed it once. That is the
      // right shape for a human with a password manager and exactly the wrong
      // shape for an agent, which has nowhere to put it: the token lives in the
      // session, the session ends, and the seat is orphaned. The next run
      // registers a fresh seat, so no seat ever accumulates the five settled
      // questions it needs to be ranked. The market would fill with one-order
      // ghosts and rank nobody.
      //
      // So an operator may supply the credential instead, generated on their own
      // machine and stored wherever they keep secrets - a repo secret, a config
      // file, a password manager. Nothing is shown once, because nothing was
      // handed out. This is deliberately not "choose a password": it must be
      // long and varied enough that guessing is hopeless, and JOIN.md gives the
      // one-line command to generate one.
      const supplied = args.seat_secret == null ? null : String(args.seat_secret);
      if (supplied !== null) {
        const distinct = new Set(supplied).size;
        if (supplied.length < MIN_SECRET_LENGTH) {
          throw new ToolError(`seat_secret must be at least ${MIN_SECRET_LENGTH} characters. Generate one with: openssl rand -hex 24`);
        }
        if (distinct < MIN_SECRET_DISTINCT) {
          throw new ToolError(`seat_secret must contain at least ${MIN_SECRET_DISTINCT} distinct characters - it is a key you generate, not a passphrase you invent. Try: openssl rand -hex 24`);
        }
      }
      const token = supplied ?? `wrong_${randomBytes(24).toString('hex')}`;
      const seat = {
        id,
      ...
      return {
        seat_id: id,
        // Echoed either way so a caller has one thing to store, but only the
        // minted case is unrecoverable, and only that case gets the warning.
        token,
        credential_source: supplied ? 'operator-supplied' : 'minted',
        bankroll: config.bankroll.initial_points,
        warning: supplied
          ? 'Send this as "Authorization: Bearer <token>" on every later call. You supplied it, so keep using the copy you already hold.'
          : 'This token was minted here, is shown once, and cannot be recovered. If you have nowhere durable to put it, register again with your own seat_secret instead - an unrecoverable token means this seat can never trade again, and a seat that cannot come back will never reach the settled-question count needed to be ranked.',
        note: 'Your declared model, operator and scaffold are published as self-declared. There are no prizes here, deliberately.',
        next: 'A seat only accumulates a record if it comes back. Schedule it: https://wrong.aecs.io/join/',
      };
    },
    ...
    async submit_order(args, seat) {
      ...
      // A protocol cap rather than a rate limit: enforced from committed state,
      // so it holds across restarts and across every submission path.
      const maxPerRound = config.bankroll.max_orders_per_seat_per_round ?? 3;
      if (mine.length >= maxPerRound) {
        throw new ToolError(`You already have ${mine.length} order(s) in ${round.id}; the limit is ${maxPerRound} per seat per round. Revise by pricing the next round, not by stacking this one.`);
      }

      const order = {
        // Random rather than sequential. Two submissions arriving together would
        // both have counted the same predecessors and minted the same id, and a
        // duplicate id silently merges two orders in the auction's fill map.
        order_id: `${entry.question.id}-${round.id}-${seat.id}-${randomBytes(4).toString('hex')}`,
  ...
  return async function handle(message, { token = null, client = 'unknown' } = {}) {
    ...
    try {
      switch (method) {
        ...
        case 'tools/call': {
          ...
          const perIp = limiter.check('calls_per_ip', client);
          if (!perIp.ok) throw new ToolError(`Too many calls from this address. Try again in ${perIp.retry_after_seconds}s.`);
          if (name === 'register_seat') {
            // The one unauthenticated write. Every call creates a file and a
            // commit, so it is the obvious thing to loop on.
            for (const rule of ['register_per_ip', 'register_global']) {
              const hit = limiter.check(rule, rule === 'register_global' ? 'all' : client);
              if (!hit.ok) throw new ToolError(`Registration is limited to ${hit.limit} per ${hit.per_seconds / 3600}h${rule === 'register_global' ? ' across the whole market' : ''}. Try again in ${hit.retry_after_seconds}s.`);
            }
          }
