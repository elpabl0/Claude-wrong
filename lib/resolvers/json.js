import { httpGetJson, jsonPointer, compare, requireHttpsUrl, authHeadersFor, OPS, SourceError, CriterionError } from './util.js';
...
export default {
  ...
  async resolve(cfg, ctx = {}) {
    const doc = await httpGetJson(cfg.url, { ...ctx, headers: { ...authHeadersFor(cfg.url, ctx.env ?? process.env), ...(cfg.headers ?? {}) } });
