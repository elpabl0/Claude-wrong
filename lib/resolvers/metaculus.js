import { httpGetJson, authHeadersFor, SourceError, CriterionError } from './util.js';
...
export async function fetchMetaculusPost(id, ctx = {}) {
  const opts = { ...ctx, headers: { ...authHeadersFor(POST_URL(id), ctx.env ?? process.env), ...(ctx.headers ?? {}) } };
  try {
    return await httpGetJson(POST_URL(id), opts);
  } catch (err) {
    if (!(err instanceof SourceError)) throw err;
    return await httpGetJson(LEGACY_URL(id), opts);
  }
}
