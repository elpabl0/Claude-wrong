...
const ORIGINS = ['self', 'crowd-mirror'];
const CROWD_PLATFORMS = ['metaculus', 'manifold'];
...
export function validatePrediction(rec, { config = loadConfig(), filename = null } = {}) {
  ...
  if (rec.origin === 'crowd-mirror') {
    const x = rec.external_reference;
    if (!x || typeof x !== 'object') {
      c.fail('`external_reference` is required when `origin` is crowd-mirror');
    } else {
      const xc = new Check(`${c.label}.external_reference`);
      xc.oneOf(x, 'platform', CROWD_PLATFORMS);
      ...
      // The resolver must point at the very question whose crowd number was
      // recorded. Mirroring one market and resolving against another would make
      // the comparison meaningless in a way nobody would ever notice.
      if (rec.resolver?.type !== x.platform) {
        c.fail(`a crowd-mirror prediction on ${x.platform} must use the \`${x.platform}\` resolver (got \`${rec.resolver?.type}\`)`);
      } else {
    ...
    c.fail('`external_reference` must be null unless `origin` is crowd-mirror');
  }
...
export { ID_RE, DATE_RE, DATETIME_RE, CLAIM_TYPES, ORIGINS, CROWD_PLATFORMS, RESOLUTION_STATUSES };
