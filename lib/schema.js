...
export function validateQuestion(rec, { config = loadConfig(), filename = null } = {}) {
  ...
  // A lane is declared, and the horizon must actually sit inside it. Without
  // this, a question could be filed in whichever lane suited it - a two-day
  // question labelled `standard` to sit on the long table, or a ninety-day one
  // labelled `canary` to keep a bad result out of the scores.
  const lanes = Object.fromEntries(Object.entries(config.lanes ?? {}).filter(([k]) => !k.startsWith('_')));
  const laneId = rec.lane ?? 'standard';
  if (!Object.prototype.hasOwnProperty.call(lanes, laneId)) {
    c.fail(`\`lane\` must be one of ${Object.keys(lanes).join(', ')} (got ${JSON.stringify(rec.lane)})`);
  }
  ...
  if (typeof rec.created_utc === 'string' && typeof rec.resolution_date === 'string' && DATE_RE.test(rec.resolution_date)) {
    ...
    if (span > maxDays) {
      c.fail(`horizon of ${span} days exceeds the maximum of ${maxDays}`);
    } else if (lane) {
      const [lo, hi] = lane.horizon_days;
      if (span < lo || span > hi) {
        c.fail(`horizon of ${span} days does not belong to the \`${laneId}\` lane (${lo}–${hi} days). A lane is decided by the horizon, not chosen.`);
      }
    }
  }
