...
/** Leaderboard, ranked on log score because win rate rewards picking the obvious. */
export function leaderboard(positionsBySeat, seatsById, config = loadConfig()) {
  ...
  for (const [id, positions] of positionsBySeat) {
    ...
    rows.push({
      ...
      self_declared: true,
      ...seatSummary(positions),
      selectivity: selectivity(positions),
    });
  }
  return rows.sort((a, b) => {
    if (a.questions_settled === 0 && b.questions_settled === 0) return a.seat.localeCompare(b.seat);
    return (b.log_score ?? -Infinity) - (a.log_score ?? -Infinity);
  });
}
