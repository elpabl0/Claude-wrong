...
export function mirrorComparison(entries) {
  const pairs = entries.filter(
    (e) =>
      e.prediction.origin === 'crowd-mirror' &&
      e.outcome !== null &&
  ...
  return {
    ...
    pairs: pairs.map((e, i) => ({
      ...
      outcome: e.outcome,
      platform: e.prediction.external_reference.platform ?? null,
...
export function fullReport(ledger) {
  ...
  return {
    ...
    by_crowd_platform: breakdown(scored, (e) => e.prediction.external_reference?.platform ?? null),
