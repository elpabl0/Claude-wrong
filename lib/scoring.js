...
export function calibration(positions, config = loadConfig()) {
  const scored = scoredPositions(positions, config).filter((p) => p.outcome !== null && p.stated_probability !== null);
 ...
 */
export function errorCorrelationMatrix(positionsBySeat, { minShared = 5, config = loadConfig() } = {}) {
  const seats = [...positionsBySeat.keys()].sort();
  const errorByQuestion = new Map();
  for (const s of seats) {
    const m = new Map();
    for (const p of scoredPositions(positionsBySeat.get(s), config)) {
...
/**
 * Positions that are allowed to count toward a score.
 *
 * A lane can be declared unscored in config, and that declaration is honoured in
 * exactly one place - here - so there is no route by which an unscored question
 * reaches a leaderboard, a calibration curve or a correlation matrix.
 */
export function scoredPositions(positions, config = loadConfig()) {
  const excluded = new Set(config.scoring.exclude_lanes ?? []);
  return positions.filter((p) => p.scored !== false && !excluded.has(p.lane ?? 'standard'));
}
 ...
 *
 * Positions in an unscored lane are dropped before anything is computed. The
 * canary lane exists to prove the pipeline runs daily; letting its deliberately
 * easy questions into a score would corrupt the very number it is protecting.
 */
export function leaderboard(positionsBySeat, seatsById, config = loadConfig()) {
  ...
  const rows = [];
  for (const [id, all] of positionsBySeat) {
    const positions = scoredPositions(all, config);
...
/**
 * The same leaderboard, split by how far ahead the question was asked.
 *
 * This exists because the pooled table is confounded and cannot be fixed by
 * sorting it differently. A two-day question is easier to be right about than a
 * ninety-day one, so a seat that trades only short questions earns a better log
 * score per contract than one trading long ones - not by forecasting better, but
 * by choosing easier questions. Pooled, that difference is invisible and reads
 * as skill.
 *
 * Splitting does not make the comparison perfect; it makes the confound visible,
 * which is the most an honest table can do. The pooled board is still published,
 * labelled, because it is what most readers look for first.
 */
export function leaderboardByHorizon(positionsBySeat, seatsById, config = loadConfig()) {
  const buckets = config.scoring.horizon_buckets;
  return buckets.map((bucket) => {
    const scoped = new Map();
    for (const [id, positions] of positionsBySeat) {
      const inBucket = scoredPositions(positions, config).filter((p) => p.horizon_bucket === bucket.id);
      if (inBucket.length) scoped.set(id, inBucket);
    }
    return {
      ...bucket,
      seats: scoped.size,
      // Recursion is intentional: one definition of a row, one definition of the
      // sort. A second copy would drift from the first the first time either changed.
      rows: leaderboard(scoped, seatsById, config),
    };
  });
}
